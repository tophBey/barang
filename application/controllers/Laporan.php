...
class Laporan extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if (isset($input['tanggal']) && is_null($input['tanggal'])) {
            $input['tanggal'] = ''; 
        }
        $this->form_validation->set_data($input);
        $this->form_validation->set_rules('transaksi', 'Transaksi', 'required|in_list[barang_masuk,barang_keluar]' ,
            [
                'required' => 'Pilihan %s harus diisi.',
                'in_list' => 'Pilihan %s tidak valid.'
            ]);
        $this->form_validation->set_rules('tanggal', 'Periode Tanggal', 'required');
        // $this->form_validation->set_rules('radio', 'Pilih Satu', 'required|callback_trim_value');
